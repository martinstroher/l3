...
namespace planopt_heuristics
{
    ...
    void AndOrGraph::most_conservative_valuation()
    {
        ...
        */
        ...
        unordered_set<NodeID> expanded_nodes;

        while (!queue.empty())
        {

            auto current_node = get_node(queue.front());
            queue.pop_front();

            if (expanded_nodes.find(current_node.id) != expanded_nodes.end())
            {
                continue;
            }
            ...
            expanded_nodes.insert(current_node.id);

            for (NodeID predecessor_id : current_node.predecessor_ids)
            {
                nodes[predecessor_id].num_forced_successors += 1;

                auto predecessor_node = get_node(predecessor_id);

                if (NodeType::OR == predecessor_node.type)
                {
                    if (expanded_nodes.find(predecessor_id) == expanded_nodes.end())
                    {
                        queue.push_back(predecessor_id);
                    }
                }
                else if (NodeType::AND == predecessor_node.type && predecessor_node.num_forced_successors == (int)predecessor_node.successor_ids.size())
                {
                    if (expanded_nodes.find(predecessor_id) == expanded_nodes.end())
                    {
                        queue.push_back(predecessor_id);
    ...
    void AndOrGraph::weighted_most_conservative_valuation()
    {
        ...
        */
        ...
        {
            bool operator()(const AndOrGraphNode &a, const AndOrGraphNode &b) const
            {
                return a.additive_cost > b.additive_cost;
            }
        };

        auto priorityQueue = priority_queue<AndOrGraphNode, vector<AndOrGraphNode>, CostComparator>();
        ...
        for (auto &node : nodes)
        {
            ...
            if (node.type == NodeType::AND && node.successor_ids.empty())
            {
                ...
                priorityQueue.emplace(node);
            }
            else
            {
                node.additive_cost = INT_MAX;
            }
        }

        auto calculateAndNodeCost = [&](AndOrGraphNode &node)
        {
            int totalCost = node.direct_cost;
            for (const auto &successor_id : node.successor_ids)
            {
                totalCost += get_node(successor_id).additive_cost;
            }
            return totalCost;
        };
        ...
        {
            auto topNode = priorityQueue.top();
            priorityQueue.pop();

            topNode.forced_true = true;

            for (auto &pred_id : topNode.predecessor_ids)
            {
                auto &predecessor = nodes[pred_id];
                predecessor.num_forced_successors++;

                if (predecessor.type == NodeType::AND &&
                    predecessor.num_forced_successors == static_cast<int>(predecessor.successor_ids.size()))
                {
                    // To compute hMax we would get all costs of the successors of the current predecessor_node and set the addictive cost of nodes from the predecessor to MAX.
                    predecessor.additive_cost = calculateAndNodeCost(predecessor);
                    priorityQueue.emplace(predecessor);
                }
                else if (predecessor.type == NodeType::OR)
                {
                    int potentialCost = topNode.additive_cost + predecessor.direct_cost;
                    if (predecessor.additive_cost > potentialCost)
                    {
                        predecessor.additive_cost = potentialCost;
                        priorityQueue.emplace(predecessor);

                        predecessor.achiever = topNode.id;
                    }
                }
            }
        }
